...
class ChunkedGenerator:
    ...
    def __init__(
        ...
        for i in range(len(poses_2d)):
            ...
            if augment:
                pairs += zip(
                    np.repeat(i, len(bounds - 1)), bounds[:-1], bounds[1:], ~augment_vector
                )
        ...
        while enabled:
            ...
            for b_i in range(start_idx, self.num_batches):
                ...
                if self.poses_3d is None and self.cameras is None:
                    ...
                    yield self.batch_cam[: len(chunks)], self.batch_3d[
                        : len(chunks)
                    ], self.batch_2d[: len(chunks)]
    ...
    def next_epoch(self):
        for seq_cam, seq_3d, seq_2d in zip_longest(self.cameras, self.poses_3d, self.poses_2d):
            ...
            if self.augment:
                ...
                batch_2d[1, :, :, 0] *= -1
                batch_2d[1, :, self.kps_left + self.kps_right] = batch_2d[
                    1, :, self.kps_right + self.kps_left
                ]
