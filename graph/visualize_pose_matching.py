...
def visualize_graph_pairs_from_json(json_file_path):
    ...
    for img_id in range(num_imgs):
        ...
        image_path = os.path.join(
            python_data[img_id]["image"]["folder"], python_data[img_id]["image"]["name"]
        )
...
def visualize_graph_pairs(candidate_dict_list_sorted, track_id):
    ...
    for dict_id in range(num_dicts - 5):
        ...
        concat_img, flag_match = visualize_graph_matching(
            candidate_dict_curr, graph_curr, candidate_dict_next, graph_next
        )
        ...
        img_name = (
            match_str
            + "_"
            + str(candidate_dict_curr["img_id"])
            + "_"
            + str(candidate_dict_next["img_id"])
            + "_"
            + str(track_id)
            + ".jpg"
        )
